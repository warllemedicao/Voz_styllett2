...
def patch_pytorch_compatibility(style_dir: Path) -> None:
    """Aplica patch no StyleTTS2 para compatibilidade com PyTorch 2.6+ (Colab Python 3.12)."""
    models_py = style_dir / "models.py"
    if not models_py.exists():
        print("[AVISO] models.py não encontrado para aplicação de patch.")
        return

    print("[INFO] Aplicando patch de compatibilidade PyTorch 2.6+...")
    content = models_py.read_text(encoding="utf-8")
    
    # Adiciona import de torch se não houver (deve haver, mas garantimos)
    # E adiciona a configuração de segurança do torch.load
    patch_code = "\n# Patch de compatibilidade PyTorch 2.6+\nimport torch\ntry:\n    from torch.serialization import add_safe_globals\n    import _codecs\n    add_safe_globals([getattr, _codecs.encode])\nexcept:\n    pass\n\n"
    
    if "Patch de compatibilidade PyTorch 2.6+" not in content:
        with models_py.open("w", encoding="utf-8") as f:
            f.write(patch_code + content)
        print("✅ Patch aplicado com sucesso em models.py")
    else:
        print("ℹ️ Patch já existente em models.py")
...
def main() -> int:
    ...
    # APLICA PATCH DE COMPATIBILIDADE LOGO APÓS CLONAR
