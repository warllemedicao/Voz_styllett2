...
def prepare_local_audio(project_dir: Path, cfg: dict) -> tuple[Path, bool]:
    ...
    for item in raw_candidates:
        ...
        print(f"- {path} | existe={path.exists()} | audios={total}")
    ...
    for item in processed_candidates:
        ...
        print(f"- {path} | existe={path.exists()} | arquivos_validos={total} | train.txt={(path / 'train.txt').exists()}")
    ...
    if not imported_processed:
        if not any(local_raw.rglob("*")):
            ...
            raise FileNotFoundError(
                "Nenhum audio bruto/processado foi encontrado no Drive.\n"
                "Crie uma destas pastas com audios ou com train.txt:\n"
                f"{tried}"
            )
        run([
            sys.executable,
